...
car = []
with open("D:/python course/assignment 17/cars_dataset.csv", "r") as file:
...
try:
    connect = sqlite3.connect("assignment 17/cardata.db")
    data = connect.cursor() 
    data.execute("""
        CREATE TABLE cars (
                Car TEXT,
...
for i in range(len(car)):
    data.execute("INSERT INTO cars VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", car[i])
